...
global function OpenAdvocateGift
...
void function InitInboxFrontMenu()
{
	...
	Hud_AddEventHandler( file.lootButton, UIE_GET_FOCUS, OnLootButton_Focused )
	Hud_AddEventHandler( file.lootButton, UIE_LOSE_FOCUS, OnLootButton_Unfocused )
...
void function OpenAdvocateGift()
{
	if ( Hud_IsLocked( file.lootButton ) )
		return

	ClientCommand( "UnlockRandomLoot" )
	EmitUISound( "Menu_AdvocateGift_Open" )

	// thread RunClientScript("TryOpenGift")
}
...
void function OnLootButton_Activate( var button )
{
	...
	OpenAdvocateGift()
	thread RunClientScript("SwitchGiftOpenStatus")
}

void function OnLootButton_Focused( var button )
{
	if ( Hud_IsLocked( button ) )
		return

	thread RunClientScript("GiftHarvesterFocused")
}

void function OnLootButton_Unfocused( var button )
{
	if ( Hud_IsLocked( button ) )
		return

	thread RunClientScript("GiftHarvesterUnfocused")
}
